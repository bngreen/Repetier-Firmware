...
// Hack to make 72 MHz STM32F1 clock work without changes to pre-existing code
...
#define PWM_TIMER               TIM2//TC0
#define PWM_TIMER_CHANNEL       1
#define PWM_TIMER_IRQ           TIM2_IRQn
#define PWM_TIMER_VECTOR        TIM2_IRQHandler
#define TIMER1_TIMER            TIM3//TC2
#define TIMER1_TIMER_CHANNEL    2
#define TIMER1_TIMER_IRQ        TIM3_IRQn
#define TIMER1_COMPA_VECTOR     TIM3_IRQHandler
...
#define	READ(pin)  GPIORead(pin)
//#define	WRITE(pin, v) PIO_SetOutput(g_APinDescription[pin].pPort, g_APinDescription[pin].ulPin, v, 0, PIO_PULLUP)
#define	WRITE(pin, v) GPIOWrite(pin, v)
 
#define	SET_INPUT(pin) GPIOSetInput(pin)
#define	SET_OUTPUT(pin) GPIOSetOutput(pin)
#define TOGGLE(pin) GPIOToggle(pin)
#define LOW         0
#define HIGH        1

#define PULLUP(pin, l) GPIOSetInput(pin, true, l==HIGH)
...
__attribute__( ( always_inline ) ) __STATIC_INLINE void __set_BASEPRI2(uint32_t value)
{
...
class InterruptProtectedBlock {
    public:
    inline void protect() {
        __set_BASEPRI2(NVIC_EncodePriority(4, 3, 0));
    }
...
#define ANALOG_INPUT_SAMPLE 9//6
...
//       WRITE(SDSS, HIGH);
   ...
   static void spiSendBlock(uint8_t token, const uint8_t* buf);
    ...
    inline static void startWatchdog() {
    	...
    	IWDG_SetReload(40000/2500);//1250);
    	IWDG_ReloadCounter();
    	IWDG_Enable();
#endif
